...
5. Run [super_raster.py](gee/super_raster.py) to generate the urban extent rasters. You need to create a tracking sheet to track the task status for each city. Update the `CITY_TRACKER` in [config.py](gee/config.py) for your tracking sheets. For each year, the initial tracking sheet should look like this:
